...
export function PlanGenerator({ onPlanGenerated }: PlanGeneratorProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      // We don't need to store the plan here, just generate it and send it for approval.
      await generatePersonalizedTrainingPlan(values)
      
      // The plan has been saved in the backend (mocked for now) and is pending approval.
      toast({
  ...
  const PlanGeneratedConfirmation = () => (
    <div className="flex flex-col items-center justify-center text-center p-8 space-y-4 h-full">
        <CheckCircle className="h-16 w-16 text-green-500 animate-pulse" />
        <h3 className="text-xl font-bold font-headline">¡Plan Generado!</h3>
        <p className="text-muted-foreground">
            Tu plan personalizado ha sido creado y enviado a Kali Gym para su revisión.
